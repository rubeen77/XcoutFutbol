"""
Escudos Bundesliga — obtiene las URLs de escudo de los 18 equipos
de Bundesliga desde Transfermarkt y actualiza escudo_url en Supabase.

La URL del escudo se deriva del ID de equipo de TM:
  https://tmssl.akamaized.net/images/wappen/normal/{tm_id}.png

Misma estrategia que escudos_premier.py, apuntando a L1.

Uso (desde backend/):
  conda activate xcout
  python scrapers/escudos_bundesliga.py
"""
...
TM_LIGA_CODE = "L1"           # código Bundesliga en Transfermarkt
TM_SEASON    = 2025
...
NOMBRE_OVERRIDE = {
    "borussia monchengladbach": "18",    # TM: "Borussia Mönchengladbach"
    "1. fc heidenheim 1846":   "2036",   # TM: "1. FC Heidenheim 1846"
    "tsg hoffenheim":          "533",    # TM: "TSG 1899 Hoffenheim"
    "1. fsv mainz 05":         "793",    # TM: "1. FSV Mainz 05"
    "fc st. pauli":            "35",     # TM: "FC St. Pauli"
}
...
# ---------------------------------------------------------------------------
# Paso 1: obtener IDs desde la página de Bundesliga en TM
# ---------------------------------------------------------------------------

def get_tm_teams() -> list[dict]:
    log.info("[1/3] Obteniendo equipos de Bundesliga desde Transfermarkt...")
    url = f"{TM_BASE}/bundesliga/startseite/wettbewerb/{TM_LIGA_CODE}/saison_id/{TM_SEASON}"
    log.info("      URL: %s", url)
    soup = _get(url)
    if not soup:
        raise RuntimeError("No se pudo cargar la pagina de Bundesliga en Transfermarkt")
...
# ---------------------------------------------------------------------------
# Paso 2: cruzar con equipos de Supabase (liga_id=25)
# ---------------------------------------------------------------------------

def match_teams(tm_teams: list[dict]) -> list[dict]:
    log.info("[2/3] Cruzando con equipos Bundesliga en Supabase (liga_id=%d, temporada=%s)...",
             LIGA_ID, TEMPORADA)
    res = (
    ...
    for s in supa_teams:
        ...
        if best:
            ...
            log.info("  OK  %-32s  <->  %-32s  (ID %s)", s["nombre"], best["name"], best["tm_id"])
        else:
            log.warning("  !!  Sin match para: %s", s["nombre"])

    unmatched = len(supa_teams) - len(matched)
    if unmatched:
        log.warning("      %d equipo(s) sin match — revisar NOMBRE_OVERRIDE.", unmatched)
    return matched
...
def update_escudos(matched: list[dict]) -> int:
    ...
    for m in matched:
        supabase.table("equipos") \
            ...
            .execute()
        n += 1
        log.info("      %-32s  -> %s", m["nombre"], m["escudo_url"])
...
def run():
    log.info("=" * 60)
    log.info(" Escudos Bundesliga -- liga_id=%d  temporada=%s", LIGA_ID, TEMPORADA)
    log.info("=" * 60)
    ...
    log.info("  Matches            : %d / 18", len(matched))
