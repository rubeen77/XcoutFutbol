"""
Transfermarkt Scraper — fotos y valor de mercado para Serie A 2025/26

Fuente: Transfermarkt.es (cloudscraper maneja Cloudflare + browser fingerprint)

Estrategia (minimo de requests):
  1.  1 request  -> Serie A page -> 20 IDs de equipo
  2. 20 requests -> plantilla de cada equipo -> nombre, foto, valor actual
  3. Cruce por nombre normalizado con jugadores de Serie A en Supabase
...
Uso (desde backend/):
  ...
  python scrapers/transfermarkt_seriea.py
"""
...
LIGA_ID          = 26
TM_SEASON        = 2025
TM_LIGA_CODE     = "IT1"      # código Serie A en Transfermarkt
...
# ---------------------------------------------------------------------------
# Paso 1: obtener IDs y slugs de los 20 equipos de la Serie A
# ---------------------------------------------------------------------------

def get_team_ids() -> list[dict]:
    log.info("[1/5] Obteniendo equipos de Serie A desde Transfermarkt...")
    url = f"{TM_BASE}/serie-a/startseite/wettbewerb/{TM_LIGA_CODE}/saison_id/{TM_SEASON}"
    log.info("      URL: %s", url)
    soup = _get(url)
    if not soup:
        raise RuntimeError("No se pudo cargar la pagina de Serie A en Transfermarkt")
...
# ---------------------------------------------------------------------------
# Paso 4: cruce con jugadores de Serie A en Supabase
# ---------------------------------------------------------------------------

def load_supabase_jugadores() -> pd.DataFrame:
    log.info("[3/5] Leyendo jugadores Serie A desde Supabase (liga_id=%d)...", LIGA_ID)
    ...
    log.info("      %d jugadores Serie A en Supabase.", len(df))
    return df
...
def run():
    log.info("=" * 58)
    log.info(" Transfermarkt Scraper -- Serie A %s  (liga_id=%d)", TEMPORADA, LIGA_ID)
    log.info("=" * 58)
