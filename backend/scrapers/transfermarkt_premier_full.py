"""
Transfermarkt Scraper — fotos y valor de mercado para Premier League 2025/26

Fuente: Transfermarkt.es (cloudscraper maneja Cloudflare + browser fingerprint)

Estrategia (minimo de requests):
  1.  1 request  -> Premier League page -> 20 IDs de equipo
  2. 20 requests -> plantilla de cada equipo -> nombre, foto, valor actual
  3. Cruce por nombre normalizado con jugadores de Premier League en Supabase
...
Uso (desde backend/):
  ...
  python scrapers/transfermarkt_premier_full.py
"""
...
# ---------------------------------------------------------------------------
# Paso 1: obtener IDs y slugs de los 20 equipos de Premier League
# ---------------------------------------------------------------------------

def get_team_ids() -> list[dict]:
    log.info("[1/5] Obteniendo equipos de Premier League desde Transfermarkt...")
    url = f"{TM_BASE}/premier-league/startseite/wettbewerb/{TM_LIGA_CODE}/saison_id/{TM_SEASON}"
    log.info("      URL: %s", url)
    soup = _get(url)
    if not soup:
        raise RuntimeError("No se pudo cargar la pagina de Premier League en Transfermarkt")
...
# ---------------------------------------------------------------------------
# Paso 4: cruce con jugadores de Premier League en Supabase
# ---------------------------------------------------------------------------

def load_supabase_jugadores() -> pd.DataFrame:
    log.info("[3/5] Leyendo jugadores Premier League desde Supabase (liga_id=%d)...", LIGA_ID)
    ...
    log.info("      %d equipos Premier League.", len(eq_ids))
    ...
    log.info("      %d jugadores Premier League en Supabase.", len(df))
    return df
...
def run():
    log.info("=" * 60)
    log.info(" Transfermarkt Scraper -- Premier League %s  (liga_id=%d)", TEMPORADA, LIGA_ID)
    log.info("=" * 60)
