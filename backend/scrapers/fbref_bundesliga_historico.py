"""
FBref Bundesliga Historical Scraper
...
Solo inserta filas para jugadores que YA existen en Supabase con liga_id=25.
...
Uso (desde backend/):
  ...
  python scrapers/fbref_bundesliga_historico.py
  python scrapers/fbref_bundesliga_historico.py --temporada 2425
"""
...
FBREF_LIGA  = "GER-Bundesliga"
LIGA_ID     = 25
MIN_MINUTOS = 90   # ignorar jugadores con menos de 90 min en la temporada
...
# ---------------------------------------------------------------------------
# Cargar mapa de jugadores Bundesliga desde Supabase
# ---------------------------------------------------------------------------

def load_bundesliga_jugadores() -> tuple[dict, dict, dict]:
    """
    Carga todos los jugadores con equipo de liga_id=25.
    Devuelve:
      full_map: {(nombre_norm, equipo_norm): jugador_id}
      name_map: {nombre_norm: jugador_id}  (primer match si hay ambiguedad)
      last_map: {apellido_norm: jugador_id}
    """
    log.info("[DB] Cargando jugadores Bundesliga (liga_id=%d)...", LIGA_ID)
...
# ---------------------------------------------------------------------------
# FBref: descargar standard stats para una temporada de Bundesliga
# ---------------------------------------------------------------------------
...
def insert_estadisticas(rows: list[dict]) -> int:
    """
    INSERT ignorando conflictos (jugador_id, temporada, liga_id).
    No sobreescribe filas ya existentes.
    """
    if not rows:
        log.info("    Nada que insertar.")
        return 0

    # Deduplicar por (jugador_id, temporada) — FBref genera filas extra
    # para jugadores que cambiaron de equipo mid-season. Conservar la ultima
    # fila (suele ser la agregada con totales de toda la temporada).
    seen: dict[tuple, int] = {}
...
def run(solo_temporada: str | None = None):
    ...
    log.info("=" * 64)
    log.info(" FBref Bundesliga Historico  (liga_id=%d)", LIGA_ID)
    log.info(" Temporadas: %s", [t for _, t in temporadas])
    log.info(" Metricas  : goles, asistencias, minutos, g90, a90, ga90")
    log.info("=" * 64)

    full_map, name_map, last_map = load_bundesliga_jugadores()
    if not name_map:
        log.error("Sin jugadores Bundesliga en DB. Ejecuta fbref_bundesliga.py primero.")
        return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="FBref historico Bundesliga — solo standard stats")
    parser.add_argument(
        "--temporada",
        metavar="CLAVE",
