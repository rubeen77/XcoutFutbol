"""
Transfermarkt Scraper — fotos y valor de mercado para Bundesliga 2025/26

Fuente: Transfermarkt.es (cloudscraper maneja Cloudflare + browser fingerprint)

Estrategia (minimo de requests):
  1.  1 request  -> Bundesliga page -> 18 IDs de equipo
  2. 18 requests -> plantilla de cada equipo -> nombre, foto, valor actual
  3. Cruce por nombre normalizado con jugadores de Bundesliga en Supabase
  4.  1 request  -> upsert batch jugadores (foto + valor)
  5.  1 request  -> upsert batch valor_mercado_historia (temporada actual)
  Total: ~21 requests HTTP
...
Uso (desde backend/):
  ...
  python scrapers/transfermarkt_bundesliga.py
"""
...
# ---------------------------------------------------------------------------
# Paso 1: obtener IDs y slugs de los 18 equipos de la Bundesliga
# ---------------------------------------------------------------------------

def get_team_ids() -> list[dict]:
    log.info("[1/5] Obteniendo equipos de Bundesliga desde Transfermarkt...")
...
# ---------------------------------------------------------------------------
# Paso 4: cruce con jugadores de Bundesliga en Supabase
# ---------------------------------------------------------------------------

def load_supabase_jugadores() -> pd.DataFrame:
    log.info("[3/5] Leyendo jugadores Bundesliga desde Supabase (liga_id=%d)...", LIGA_ID)
    ...
    log.info("      %d jugadores Bundesliga en Supabase.", len(df))
    return df
...
def run():
    log.info("=" * 56)
    log.info(" Transfermarkt Scraper -- Bundesliga %s  (liga_id=%d)", TEMPORADA, LIGA_ID)
    log.info("=" * 56)
