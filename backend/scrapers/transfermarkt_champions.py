"""
Transfermarkt Scraper — fotos y valor de mercado para Champions League 2025/26

Fuente: Transfermarkt.es (cloudscraper maneja Cloudflare + browser fingerprint)

Estrategia:
  1.  Lista hardcoded de 29 equipos UCL 25/26 (sin request inicial)
  2.  N requests -> plantilla de cada equipo -> nombre, foto, valor actual
  3.  Cruce por nombre normalizado con jugadores de UCL en Supabase
  4.  Upsert batch jugadores (foto + valor)
  5.  Upsert historial valor_mercado_historia

Uso (desde backend/):
  conda activate xcout
  python scrapers/transfermarkt_champions.py
"""
...
# ---------------------------------------------------------------------------
# Paso 1: lista hardcoded de equipos UCL 25/26
# ---------------------------------------------------------------------------

# (slug, tm_id, nombre_display)
_UCL_TEAMS = [
    ("paris-saint-germain",    583,   "PSG"),
    ("fc-arsenal",              11,   "Arsenal"),
    ("inter-mailand",           46,   "Inter Milan"),
    ("atletico-madrid",         13,   "Atlético Madrid"),
    ("bayer-04-leverkusen",     15,   "Bayer Leverkusen"),
    ("aston-villa",            405,   "Aston Villa"),
    ("borussia-dortmund",       16,   "Borussia Dortmund"),
    ("losc-lille",            1082,   "Lille"),
    ("feyenoord-rotterdam",    234,   "Feyenoord"),
    ("fc-brugge",             2282,   "Club Brugge"),
    ("sl-benfica",             294,   "Benfica"),
    ("sporting-cp",            336,   "Sporting CP"),
    ("celtic-glasgow",         371,   "Celtic"),
    ("juventus-turin",         506,   "Juventus"),
    ("atalanta-bc",            800,   "Atalanta"),
    ("sk-sturm-graz",         1635,   "Sturm Graz"),
    ("psv-eindhoven",          383,   "PSV"),
    ("rasenballsport-leipzig", 23826,  "RB Leipzig"),
    ("as-monaco",              162,   "Monaco"),
    ("shakhtar-donetsk",       669,   "Shakhtar Donetsk"),
    ("girona-fc",            12321,   "Girona"),
    ("gnk-dinamo-zagreb",      419,   "Dinamo Zagreb"),
    ("stade-brestois-29",     3911,   "Brest"),
    ("sk-slovan-bratislava",  1428,   "Slovan Bratislava"),
    ("roter-stern-belgrad",    581,   "Red Star Belgrade"),
    ("fc-bologna",            1025,   "Bologna"),
    ("vfb-stuttgart",           79,   "Stuttgart"),
    ("ac-sparta-prag",         197,   "Sparta Prague"),
    ("bsc-young-boys",        1799,   "Young Boys"),
]


def get_team_ids() -> list[dict]:
    log.info("[1/5] Usando lista hardcoded de %d equipos UCL 25/26...", len(_UCL_TEAMS))
    teams = []
    for slug, tm_id, name in _UCL_TEAMS:
        kader_url = f"{TM_BASE}/{slug}/kader/verein/{tm_id}/saison_id/{TM_SEASON}"
        teams.append({"name": name, "tm_id": str(tm_id), "kader_url": kader_url})
        log.info("      ID=%-6s  %s", tm_id, name)
    return teams
...
# ---------------------------------------------------------------------------
# Paso 4: cruce con jugadores de Champions League en Supabase
# ---------------------------------------------------------------------------

def load_supabase_jugadores() -> pd.DataFrame:
    log.info("[3/5] Leyendo jugadores Champions League desde Supabase (liga_id=%d)...", LIGA_ID)
    ...
    log.info("      %d equipos Champions League.", len(eq_ids))
    ...
    log.info("      %d jugadores Champions League en Supabase.", len(df))
    return df
...
def run():
    log.info("=" * 60)
    log.info(" Transfermarkt Scraper -- Champions League %s  (liga_id=%d)", TEMPORADA, LIGA_ID)
    log.info("=" * 60)
