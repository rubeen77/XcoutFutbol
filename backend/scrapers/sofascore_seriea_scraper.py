"""
Sofascore Stats Scraper — regates y pases% para Serie A 2025/26
...
buscando la temporada "25/26" del torneo 23 (Serie A).

Uso (desde backend/):
  python scrapers/sofascore_seriea_scraper.py
"""
...
TOURNAMENT_ID = 23      # Serie A en Sofascore
...
def discover_season_id() -> int:
    ...
    log.info("[season] Consultando temporadas Serie A en Sofascore...")
...
def fetch_all_stats(season_id: int) -> list[dict]:
    log.info("[1/4] Descargando stats de temporada desde Sofascore Serie A...")
...
def load_supabase_stats() -> list[dict]:
    log.info("[2/4] Leyendo estadisticas existentes en Supabase Serie A...")
    res = (
...
def upsert_stats(rows: list[dict]) -> int:
    log.info("[4/4] Actualizando regates y pases%% en Supabase Serie A...")
    if not rows:
...
def verify():
    ...
    for name in ("Vlahovic", "Lautaro", "Barella"):
        ...
        if est.data:
            ...
            log.info("  %-25s -> sin stats Serie A", nom)
...
def run():
    ...
    log.info("=" * 60)
    log.info(" Sofascore Serie A Stats Scraper -- %s", DB_TEMPORADA)
