"""
Sofascore Stats Scraper — regates y pases% para Champions League 2025/26
...
buscando la temporada "25/26" del torneo 7 (Champions League).

Uso (desde backend/):
  python scrapers/sofascore_champions_scraper.py
"""
...
def fetch_all_stats(season_id: int) -> list[dict]:
    log.info("[1/4] Descargando stats desde Sofascore Champions League...")
...
def load_supabase_stats() -> list[dict]:
    log.info("[2/4] Leyendo estadisticas existentes en Supabase Champions League...")
    res = (
...
def upsert_stats(rows: list[dict]) -> int:
    log.info("[4/4] Actualizando regates y pases%% en Supabase Champions League...")
    if not rows:
...
def run():
    ...
    log.info("=" * 60)
    log.info(" Sofascore Champions League Stats Scraper -- %s", DB_TEMPORADA)
