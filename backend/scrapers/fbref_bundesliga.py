"""
FBref Bundesliga Scraper — Bundesliga 2025/26 (liga_id=25)

Carga desde cero la Bundesliga en Supabase en 6 fases:
...
Uso (desde backend/):
  ...
  python scrapers/fbref_bundesliga.py
"""
...
def extract_shooting() -> list[dict]:
    ...
    # xG de FBref shooting como valor inicial (Understat lo sobreescribe en fase 5)
    col_xg = (
...
def load_partidos() -> int:
    log.info("[6] Descargando schedule Bundesliga (FBref)...")
    try:
        fbref = sd.FBref(leagues=LEAGUE_STR, seasons=TEMPORADA)
...
def verify():
    ...
    for name in ("Kane", "Musiala", "Wirtz", "Neuer", "Kimmich"):
        res = (
...
def run():
    log.info("=" * 66)
    log.info(" Bundesliga Scraper  (liga_id=%d, temporada=%s)", LIGA_ID, TEMPORADA)
    log.info(" Fases: standard -> keeper -> shooting -> misc -> Understat -> partidos")
    log.info("=" * 66)

    # ── 1. Standard: equipos + jugadores + estadisticas basicas ───────────────
