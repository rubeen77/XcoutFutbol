...
function getZona(pos, nombre, ligaId = 1) {
  ...
  if (ligaId === 25) {
    // Bundesliga: 4 CL, 1 EL, 1 Conference, 1 Playoff descenso, 2 descenso
  ...
  if (ligaId === 24) {
    // Premier League: 5 CL, 1 EL, 1 Conference, 3 descenso
  ...
  // LaLiga: 4 CL, 1 EL, 1 Conference, 3 descenso
  if (pos <= 4)  return ZONA_MAP.champions
  if (pos === 5) return ZONA_MAP.europa
  if (pos === 6) return ZONA_MAP.conference
  if (pos >= 18) return ZONA_MAP.descenso
...
function TeamCard({ equipo, selected, onClick, liga = { id: 1, nombre: 'LaLiga' } }) {
  const color = teamColor(equipo.nombre)
  const abrev = teamAbrev(equipo.nombre)
  const zona  = getZona(equipo.posicion_clasificacion, equipo.nombre, liga.id)
      ...
      <div className="p-4 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            {equipo.puntos != null ? `${equipo.puntos} pts` : `${liga.nombre} 2025/26`}
          </p>
        </div>
...
/* ── TeamModal ────────────────────────────────────────────────────────────── */
function TeamModal({ equipo, detalle, loading, onClose, liga = { id: 1 } }) {
  const zona        = getZona(equipo.posicion_clasificacion, equipo.nombre, liga.id)
...
export default function Equipos() {
  ...
  useEffect(() => {
    ...
    getEquipos(ligaId)
      .then(data => {
        const sorted = (data.equipos || []).slice().sort((a, b) => {
          if (a.posicion_clasificacion == null) return 1
          if (b.posicion_clasificacion == null) return -1
          return a.posicion_clasificacion - b.posicion_clasificacion
        })
        setListaEquipos(sorted)
      })
